...
export class MySphere extends CGFobject {
    ...
    constructor(scene, radius, slices, stacks, inside = false, maxHeightRatio = 1.0) {
        super(scene);
        this.radius = radius;
        this.slices = slices;
        this.stacks = stacks;
        this.inside = inside ? -1 : 1;
        this.maxHeightRatio = Math.max(0.0, Math.min(1.0, maxHeightRatio));
        this.initBuffers();
    }
    ...
    initBuffers() {
        ...
        const startStack = Math.floor(this.stacks * (1 - this.maxHeightRatio));

        for (let stack = startStack; stack <= this.stacks; stack++) {
        ...
        // Indices
        const stackCount = this.stacks - startStack;
