...
export class ShaderScene extends CGFscene {
	...
	init(application) {
		...
		this.testShaders = [
			...
			new CGFshader(this.gl, "shaders/texture1.vert", "shaders/ex2.frag"),
	...
	update(t) {
		if (this.selectedExampleShader in [6, 11])
	...
	display() {
		...
		if (this.selectedObject == 0) {
			this.appearance.setTexture(this.texture); // Teapot usa textura padrão
		} else if (this.selectedObject == 1) {
			this.appearance.setTexture(this.texture2); // Plane usa a segunda textura
		}
		
		// Se o shader da água for selecionado, use as texturas corretas
		if (this.selectedExampleShader == 11) {
			this.appearance.setTexture(this.textureWater);
			this.textureWater2.bind(1);
		}
